...
function ProductDetails() {
  ...
  return (
    <div className='p-6'>
        ...
        {
            return (<img onClick={()=>setMainImage(image.url)} src={image.url}
            alt={image.altText || `Thumbnail ${index}`}
            key={index}
className={`${mainImage === image.url ? "border-black" : "border-gray-300"} w-20 h-20 rounded-lg cursor-pointer`}
                ...
                {selectedProducts.images.map((image,index)=>
        {
            ...
            alt={image.altText || `Thumbnail ${index}`}
            key={index}
            className={`${mainImage===image.url?"border-black":"border-gray-300"} w-20 h-20 object cover rounded-lg cursor-pointer `}
        ...
        <div className='mb-4'>
            ...
            <div className='flex gap-2 mt-2'>
                ...
                {
                    return (<button onClick={()=>setSelectedColor(color)} style={{backgroundColor:color.toLocaleLowerCase(),filter:"brightness(0.5"}} key={index} className={`w-8 h-8 rounded-full  ${selectedColor===color? "border-4 border-black":"border-gray-300"}`}>
        ...
        <div className='mb-4'>
                ...
                <div className='flex gap-2 mt-2'>
                    {selectedProducts.sizes.map((size)=>
                    {
                        ...
                        key={size} className={`px-4 py-2 rounded ${selectedSize===size?"bg-black text-white":""}`}>
                            {size}
                        </button>)
                    })}
                </div>
